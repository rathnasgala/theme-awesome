...
Awesome is an official Gala appearance theme with expressive violet and coral color, editorial
display type, pill-shaped controls, and layered cards. It changes presentation only: Gala's
managed templates, authentication, analytics, accessibility behavior, security policy, and
...
1. Publish this repository as `rathnasgala/theme-awesome`.
...
4. Enter repository `theme-awesome`, enter the exact SHA, and select **Verify release**.
5. Authors can then select **Awesome** in their publication's Appearance settings when its
