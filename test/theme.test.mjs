...
test('ships the immutable Awesome release contract', async () => {
  const { manifest, cssBytes } = await verifyTheme();
  const packageJson = JSON.parse(await readFile(path.join(ROOT, 'package.json'), 'utf8'));

  assert.equal(manifest.id, 'awesome');
  ...
  assert.equal(packageJson.name, '@rathnasgala/theme-awesome');
...
async function withFixture(assertions) {
  const temporaryDirectory = await mkdtemp(path.join(os.tmpdir(), 'gala-awesome-'));
  const fixture = path.join(temporaryDirectory, 'theme-awesome');
  try {
    await mkdir(fixture);
